...
from models.swinT import SWIN_VARIANTS
from models.head import FCNHead
...
class SSp4cat(nn.Module):
    ...
    def __init__(self, options):
        ...
        # ---- 解码器 ---- #
        self.fcnhead_sic = FCNHead(
            in_channels=self.fuse_ch,     
            num_classes=self.nclass,      
            in_index=3,                   # 第4层（P4）
            channels=512,                 # 可按需改，与你 FCNHead 默认一致即可
            num_convs=2,
            kernel_size=3,
            concat_input=False
        )
        if self.use_iw_head:
            self.fcnhead_iw  = FCNHead(
                in_channels=self.fuse_ch, 
                num_classes=2,            
                in_index=3,
                channels=512,             # IW 可更轻些；暂时一致
                num_convs=2,
                kernel_size=3,
                concat_input=False
            )
