...
from models.cswin import CSWIN_VARIANTS
...
class DriFTNet(nn.Module):
    ...
    def __init__(self, options):
        ...
        cswin_sar_name  = options.get('cswin_sar_backbone', 'cswin_tiny')
        cswin_amsr_name = options.get('cswin_amsr_backbone', 'cswin_tiny')

        assert cswin_sar_name in CSWIN_VARIANTS, f'Unknown CSWin: {cswin_sar_name}'
        assert cswin_amsr_name in CSWIN_VARIANTS, f'Unknown CSWin: {cswin_amsr_name}'

        sar_builder  = CSWIN_VARIANTS[cswin_sar_name]
        amsr_builder = CSWIN_VARIANTS[cswin_amsr_name]
        ...
        self.sar_cswin  = sar_model.backbone
        self.amsr_cswin = amsr_model.backbone
    ...
    def forward(self, x, mode='all', **kwargs):
        ...
        # --- Swin backbone（多尺度） --- #
        u1, u2, u3, u4 = self.sar_cswin(x_sar)     # SAR 3ch
        s1, s2, s3, s4 = self.amsr_cswin(x_amsr)   # AMSR2 6ch
