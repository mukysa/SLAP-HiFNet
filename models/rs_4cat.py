...
class RS4cat(nn.Module):
    ...
    def __init__(self, options):
        ...
        # ---- UNet 编码器（SAR）---- #
        unet_encoder = options.get('unet_encoder', 'resnet34')
        unet_weights = options.get('unet_encoder_weights', 'imagenet')  # SAR 3通道可用 'imagenet'
        self.unet = smp.Unet(
        ...
        u_ch = getattr(self.unet.encoder, 'out_channels', None)
        if u_ch is None:
            # 兜底估计（大概率用不到）
            u_ch = [64, 64, 128, 256, 512]

        # ---- Swin 主干（AMSR2，6通道）---- #
        swin_name = options.get('swin_backbone', 'swin_tiny')
        assert swin_name in SWIN_VARIANTS, f'Unknown Swin variant: {swin_name}'
        img_size = options.get('sar_patch_size', 1024)
        swin_builder = SWIN_VARIANTS[swin_name]
        # 直接指定 in_chans=self.amsr_in (=6)
        swin_model = swin_builder(
            nclass=self.nclass,
            img_size=img_size,
            pretrained=options.get('pretrained', True),
            aux=False, head='none', edge_aux=False,
            in_chans=self.amsr_in     # <<< 关键：6 通道
        )
        self.swin = swin_model.backbone
        self.swin_dims = getattr(swin_model, 'head_dim', None)
        if self.swin_dims is None:
        ...
        # Swin (AMSR2)
        self.align_s1 = ConvBNReLU(self.swin_dims[-4], self.fuse_ch, 1, 1, 0)
        self.align_s2 = ConvBNReLU(self.swin_dims[-3], self.fuse_ch, 1, 1, 0)
        self.align_s3 = ConvBNReLU(self.swin_dims[-2], self.fuse_ch, 1, 1, 0)
        self.align_s4 = ConvBNReLU(self.swin_dims[-1], self.fuse_ch, 1, 1, 0)
