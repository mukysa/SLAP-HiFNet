...
def rest_tiny(nclass, img_size=None, pretrained=False, aux=False, head='uperhead', edge_aux=False):
    if pretrained:
...
def rest_small(nclass, img_size=None, pretrained=False, aux=False, head='uperhead', edge_aux=False):
    if pretrained:
...
def rest_base(nclass, img_size=None, pretrained=False, aux=False, head='uperhead', edge_aux=False):
    if pretrained:
...
def rest_large(nclass, img_size=None, pretrained=False, aux=False, head='uperhead', edge_aux=False):
    if pretrained:
...
def rest_tiny_nband(nclass, nband=3, img_size=None, pretrained=False, aux=False, head='uperhead', edge_aux=False):
    if pretrained:
        pretrained_root = './pretrained_weights/rest_lite.pth'
    else:
        pretrained_root = None
    model = ResT(
        nclass=nclass, aux=aux, head=head, edge_aux=edge_aux,
        embed_dim=[64, 128, 256, 512], num_heads=[1, 2, 4, 8],
        mlp_ratios=[4, 4, 4, 4], qkv_bias=True,
        depths=[2, 2, 2, 2], sr_ratios=[8, 4, 2, 1], apply_transform=True,
        pretrained_root=pretrained_root
    )
    model.backbone.stem = BasicStem(in_ch=nband, out_ch=64, with_pos=True)  # 修改输入通道
    return model


def rest_small_nband(nclass, nband=3, img_size=None, pretrained=False, aux=False, head='uperhead', edge_aux=False):
    if pretrained:
        pretrained_root = './pretrained_weights/rest_small.pth'
    else:
        pretrained_root = None
    model = ResT(
        ...
        embed_dim=[64, 128, 256, 512], num_heads=[1, 2, 4, 8],
        mlp_ratios=[4, 4, 4, 4], qkv_bias=True,
        depths=[2, 2, 6, 2], sr_ratios=[8, 4, 2, 1], apply_transform=True,
        pretrained_root=pretrained_root
    )
    model.backbone.stem = BasicStem(in_ch=nband, out_ch=64, with_pos=True)
    return model


def rest_base_nband(nclass, nband=3, img_size=None, pretrained=False, aux=False, head='uperhead', edge_aux=False):
    if pretrained:
        pretrained_root = './pretrained_weights/rest_base.pth'
    else:
        pretrained_root = None
    model = ResT(
        nclass=nclass, aux=aux, head=head, edge_aux=edge_aux,
        embed_dim=[96, 192, 384, 768], num_heads=[1, 2, 4, 8],
        mlp_ratios=[4, 4, 4, 4], qkv_bias=True,
        depths=[2, 2, 6, 2], sr_ratios=[8, 4, 2, 1], apply_transform=True,
        pretrained_root=pretrained_root
    )
    model.backbone.stem = BasicStem(in_ch=nband, out_ch=96, with_pos=True)
    return model


def rest_large_nband(nclass, nband=3, img_size=None, pretrained=False, aux=False, head='uperhead', edge_aux=False):
    if pretrained:
        pretrained_root = './pretrained_weights/rest_large.pth'
    else:
        pretrained_root = None
...
REST_VARIANTS = {
    'rest_tiny': rest_tiny_nband,
    'rest_small': rest_small_nband,
    'rest_base': rest_base_nband,
    'rest_large': rest_large_nband
}
