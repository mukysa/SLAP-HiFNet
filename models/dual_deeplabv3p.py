...
class SMPResNetBackbone(nn.Module):
    """
    与 smp==0.3.3 完全兼容
    不做 dilation，不修改 stride
    正常输出 low_level (H/4) 和 x (H/32)
    """
    def __init__(self, encoder_name="resnet34",
        ...
        # depth=5 => 返回 5 个尺度特征: H/2, H/4, H/8, H/16, H/32
        ...
        # 通道数（必需，让 DeepLab 的 ASPP 适配）
        self.out_channels = self.encoder.out_channels[-1]      # 最深层 512/2048
        self.low_level_channels = self.encoder.out_channels[1] # 第二层，一般是 64

    def forward(self, x):
        feats = self.encoder(x)
        low = feats[1]     # H/4
        x   = feats[-1]    # H/32（没有 dilation）
        return low, x
...
class DeepLab(nn.Module):
    def __init__(self, options):
        ...
        elif self.backbone in ["resnet18", "resnet34", "resnet50", "resnet101"]:
            #----------------------------------#
            #   ResNet 主干（smp）
            #----------------------------------#
